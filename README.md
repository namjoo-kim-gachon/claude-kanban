...
3. 요청을 `MENTION_TO_TMUX`에 매핑된 tmux pane(예: `@claude=claude:0.0`)으로 전달
...
GITHUB_PAT=
MENTION_TO_TMUX=@claude=session_name:0.0,@ops=session_name:0.1
SQLITE_PATH=./state/webhook.db
LOG_LEVEL=INFO
```

필수로 보면 되는 값:
- `MENTION_TO_TMUX`: 멘션→tmux pane 매핑 (예: `@claude=claude:0.0,@ops=claude:0.1`) **필수**
...
- 값이 비어 있으면 webhook 필터에서 멘션을 허용하지 않습니다.
