...
`claude-kanban`은 GitHub 이슈 댓글의 `@claude` 요청을 받아, **이슈 내용(제목/본문)과 함께** tmux 0번 창(예: `claude:0.0`)으로 전달해 Claude Code가 바로 작업을 시작할 수 있게 해주는 webhook 서버입니다. 클로드코드는 작업을 마치고 결과를 요약해 이슈 댓글로 남기고, 이슈 작성자를 멘션해 알려줍니다. 또한 해당 이슈가 GitHub Projects에 등록되어 있으면 작업 시작 시 자동으로 `In Progress`로 옮기고, 작업 완료 후 `Review`로 옮겨줍니다.
...
1. 이슈 댓글에 `@claude` 요청을 남김
2. 서버가 요청 문장 + 이슈 제목/본문을 함께 수집
3. 요청을 tmux **0번 창**(`TMUX_TARGET`, 예: `claude:0.0`)으로 전달
...
필수로 보면 되는 값:
- `ALLOWED_REPO`: 어떤 저장소 요청을 처리할지
- `TMUX_TARGET`: 어느 tmux 창으로 보낼지 (예: `claude:0.0`) **필수**
...
Projects 권한이 없어도 요청 전달 자체는 계속 진행됩니다.

### 2) `TMUX_TARGET` 값

- 값이 비어 있으면 worker가 요청을 실행하지 않습니다.
- 값이 틀리면 전달에 실패합니다.
